...
class FramePreprocessor:
    ...
    def __init__(self, min_height: int = 480, max_height: int = 1080):
        self.min_height = min_height
        self.max_height = max_height

        # Para o frame inteiro, parâmetros mais suaves (clipLimit menor, tileGridSize maior)
        self._clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        # Para o recorte da matrícula, parâmetros mais agressivos (clipLimit maior, tileGridSize menor)
    ...
    def process(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Denoise é sempre feito, pois o ruído piora todas as etapas seguintes
    ...
    def _resize(self, frame: np.ndarray) -> np.ndarray:
        ...
        if h < self.min_height:
            ...
            frame = cv2.resize(frame, (int(w * scale), self.min_height), interpolation=cv2.INTER_CUBIC)
        elif h > self.max_height:
            scale = self.max_height / h
            frame = cv2.resize(frame, (int(w * scale), self.max_height), interpolation=cv2.INTER_AREA)
        return frame
